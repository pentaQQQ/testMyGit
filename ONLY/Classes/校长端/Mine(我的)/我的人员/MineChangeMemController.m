...
@interface MineChangeMemController ()

@end

@implementation MineChangeMemController
{
    NSString *name;
...
- (void)setNavView
{
    ...
    if (self.isType==0) {
      titleLabel.text = @"添加人员信息";
    }
    else {
    titleLabel.text = @"修改添加人员信息";
    }
...
{
    if (self.isType==0) {
        [self.delegate Fresh:textfield.text];
        [self.navigationController popViewControllerAnimated:YES];
    }
    else {
        
        [DataSourceTool changePerson:self.person_id key:self.keyName vakue:textfield.text ViewController:self success:^(id json) {
