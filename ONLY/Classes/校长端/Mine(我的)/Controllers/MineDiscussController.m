...
@interface MineDiscussController ()<UITableViewDelegate,UITableViewDataSource,StarViewDelegate,UIImagePickerControllerDelegate,SGActionSheetDelegate,UINavigationControllerDelegate>

@end

@implementation MineDiscussController
{
...
{
    NSString *str  = [NSString stringWithFormat:@"%ld,%ld,%ld",stars1,stars2,stars3];
    [DataSourceTool addDiscussTypeId:[NSString stringWithFormat:@"%ld",neckName] comment_content:desc comment_type:self.isType comment_img:imgArrayData comment_star:str comment_id:self.goods_id ViewController:self success:^(id json) {
...
{
    static NSString *cellId1 = @"MineDiscussCell1";
    ...
    if (cell1==nil) {
        ...
        [imageview sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",Choose_Base_URL,self.goods_img]] placeholderImage:[UIImage imageNamed:@"图层59"]];
        cornerRadiusView(imageview, 3);
        imageview.sd_layout.leftSpaceToView(cell1.contentView,17).topSpaceToView(cell1.contentView,16).widthIs(107*SCREEN_PRESENT).heightIs(61);
        
        UILabel *label2 = [UILabel new];
        [cell1.contentView addSubview:label2];
        [self makeLabel:label2 font:14 color:colorWithRGB(0x333333) text:self.goods_name];
        label2.sd_layout.leftSpaceToView(imageview,12).topSpaceToView(cell1.contentView,25).heightIs(14).rightSpaceToView(cell1.contentView,5);
        
        UILabel *label4 = [UILabel new];
        [cell1.contentView addSubview:label4];
        [self makeLabel:label4 font:14 color:colorWithRGB(0x999999) text:[NSString stringWithFormat:@"x%@",self.goods_number]];
        label4.textAlignment = NSTextAlignmentRight;
        label4.sd_layout.rightSpaceToView(cell1.contentView,17).topSpaceToView(label2,17).heightIs(12).widthIs(48);
        
        UILabel *label3 = [UILabel new];
        [cell1.contentView addSubview:label3];
        [self makeLabel:label3 font:14 color:colorWithRGB(0x999999) text:[NSString stringWithFormat:@"¥%@",self.goods_price]];
    ...
    if (cell2==nil) {
        ...
        textView.tag = 200;
        textView.placeholder = @"写下对此产品的感受";
    ...
    if (cell3==nil) {
        ...
        for (int i = 0; i<3; i++) {
            ...
            StarView *starView = [[StarView alloc] initWithFrame:CGRectMake(90*SCREEN_PRESENT, 23+i*25, 200, 20) EmptyImage:@"graystar" StarImage:@"redstar"];
