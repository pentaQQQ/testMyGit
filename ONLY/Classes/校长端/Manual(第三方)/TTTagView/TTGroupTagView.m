...
- (void)commonInit {
    ...
    _tagPaddingSize = CGSizeMake(15, 15);
    // 第一个参数表示标签内部text文本左右距离文本框的距离
    _textPaddingSize = CGSizeMake(20, 0);
...
//    self.backgroundColor = kColorRGB(0xffffff);
    ...
    _bgColor =  colorWithRGB(0x333333);//默认状态下的字体颜色; // colorWithRGB(0x999999);
    _selBgColor = colorWithRGB(0xE4F3FA); //选中状态下的背景颜色;
    _textColor = [UIColor clearColor]; //默认状态下的边框颜色; //colorWithRGB(0xcccccc);
    _selTextColor = colorWithRGB(0x333333); //选中状态下的字体颜色; //colorWithRGB(0xffae00);
    _borderColor = colorWithRGB(0xf0f0f0);  //默认状态下的背景颜色;
    _selBorderColor = [UIColor clearColor]; //选中状态下的边框颜色;
    self.backgroundColor = colorWithRGB(0xffffff); //整体的背景颜色
...
- (TTTagViewCheckBoxButton *)tagButtonWithTag:(NSString *)tag {
    ...
    tagBtn.layer.cornerRadius = 4; //btnFrame.size.height * 0.5f;
    
    btnFrame.size.width = [tagBtn.titleLabel.text sizeWithAttributes:@{NSFontAttributeName:_fontTag}].width + _textPaddingSize.width*2; //(tagBtn.layer.cornerRadius * 2.0f);
