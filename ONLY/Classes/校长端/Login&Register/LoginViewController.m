...
- (void)makeUI
{
    ...
    [self makeLabel:schoolmaster font:18 color:colorWithRGB(0x333333) text:@"校长登录"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell1==nil) {
        ...
        btn2.sd_layout.rightSpaceToView(cell1.contentView,8).widthIs(102).topSpaceToView(btn,24).heightIs(25);
        ...
        [btn2 addTarget:self action:@selector(ResigiterBtnClick) forControlEvents:UIControlEventTouchUpInside];
    }
...
- (void)loginClick
{
    ...
    [DataSourceTool requestLogin:mobile password:password  type :@"1" toViewController:self success:^(id json) {
        if ([json[@"errcode"] integerValue]==0) {
           USERINFO.personal= NO;
           [DataSourceTool saveUserInfo:json[@"rsp"]];
           [UIApplication sharedApplication].keyWindow.rootViewController =[BaseTabBarController new].tabBarController;
        }
...
- (void)changeMember
{
    ...
    MemberLoginController *myVC = [[MemberLoginController alloc] init];
    //创建动画
