...
-(instancetype)initWithCoder:(NSCoder *)aDecoder{
    ...
    if (self) {
        ...
        CGFloat averageHeight = (self.frame.size.width-10*(5-1))/5;
        ...
        [self loadCustomViewWithTotal:5];
    }
    return self;
}
- (instancetype)initWithFrame:(CGRect)frame withTotalStar:(NSInteger)totalStar withTotalPoint:(CGFloat)totalPoint starSpace:(NSInteger)space
{
    ...
    if (self) {
        ...
        self.starAliment=StarAlimentCenter;
...
- (void)setCommentPoint:(CGFloat)commentPoint
{
    ...
    for (int j = 0; j< fullNumber; j++) {
        ...
        starImageView.image = [UIImage imageNamed:@"redstar"];
    ...
    if (part > 0) {
        ...
        partImage.image = [UIImage imageNamed:@"订课_评价弹层_star_selected"];
