...
@interface ServerViewController ()<UITableViewDelegate,UITableViewDataSource,UIImagePickerControllerDelegate,SGActionSheetDelegate,UINavigationControllerDelegate>
...
- (void)setNavView
{
    ...
    if(self.idx == 1){
        ...
        titleLabel.text = @"培训提议";
    }
    else
    {
        titleLabel.text = @"服务提议";
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell5==nil) {
        ...
        textView.placeholder = @"比如培训的目的、意义……";
    ...
    if (cell1==nil) {
        ...
        textField.tag = 100;
        textField.placeholder = @"用一句话描述您的提议";
...
{
    if(productName.length>0&&desc.length>0&&typeName.length>0) {

    [DataSourceTool addPXTypeId:typeID apply_name:productName apply_desc:desc apply_type:[NSString stringWithFormat:@"%ld",self.idx] file:imgArrayData ViewController:self success:^(id json){
...
- (void)loadTypeList
{
    ...
    [DataSourceTool typeList:[NSString stringWithFormat:@"%ld",self.idx] ViewController:self success:^(id json) {
