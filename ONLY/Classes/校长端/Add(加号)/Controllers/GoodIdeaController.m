//
//  GoodIdeaController.m
//  ONLY
//
//  Created by 上海点硕 on 2017/1/9.
...
#import "SupModel.h"
@interface GoodIdeaController ()<UITableViewDelegate,UITableViewDataSource,UIImagePickerControllerDelegate,SGActionSheetDelegate,UINavigationControllerDelegate>

@end

@implementation GoodIdeaController
{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell4==nil) {
        ...
        UILabel *titleLabel = [UILabel new];
        [view addSubview:titleLabel];
        titleLabel.tag = 52+indexPath.row;
        ...
        detailLabel.tag = 104+indexPath.row;
    ...
    if (cell1==nil) {
        ...
        UILabel *titleLabel = [UILabel new];
        [view addSubview:titleLabel];
        titleLabel.tag = 50+indexPath.row;
        [self makeLabel:titleLabel font:14 color:colorWithRGB(0x333333) text:@"标题"];
        ...
        textField.placeholder = @"用一句话描述您的想法";
    ...
    if (indexPath.section==0) {
        if (indexPath.row==0||indexPath.row==1) {
            ...
            UILabel *lab = [cell1.contentView viewWithTag:50+indexPath.row];
            lab.text = titleArr[indexPath.section][indexPath.row];
            
            UITextField *textField = [cell1.contentView viewWithTag:100+indexPath.row];
            if (indexPath.row==0) {
        ...
        {
            UILabel *lab = [cell4.contentView viewWithTag:52+indexPath.row];
            lab.text = titleArr[indexPath.section][indexPath.row];
            
            UILabel *detailLabel = [cell4.contentView viewWithTag:104+indexPath.row];
            if(indexPath.row==2)
            {
             detailLabel.text = typeName1;
            }
            else
            {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section==0) {
        ...
        SGActionSheet *sheet = [[SGActionSheet alloc] initWithTitle:nil delegate:self cancelButtonTitle:@"取消" otherButtonTitleArray:temp];
        sheet.cancelButtonTitleColor = [UIColor redColor];
        sheet.tag = 10001+indexPath.row;
        [sheet show];
        }
    }
...
{
    if(productName.length>0&&keywords.length>0&&desc.length>0&&typeName.length>0&&typeName1.length>0)
    {
      
        [DataSourceTool addGoodIdeaTypeId:typeID apply_name:productName apply_desc:desc apply_price:@"" keywords:keywords support_id:typeID1 file:imgArrayData ViewController:self success:^(id json) {
...
- (void)SGActionSheet:(SGActionSheet *)actionSheet didSelectRowAtIndexPath:(NSInteger)indexPath {
    ...
    {
        typeID1 =  dataArray3[indexPath];
        typeName1 = dataArray2[indexPath];
        [tableview reloadData];
    }
}
...
- (void)loadTypeList
{
    [DataSourceTool typeList:@"2" ViewController:self success:^(id json) {
...
- (void)loadTypeList1
{
   [DataSourceTool support:@"" ViewController:self success:^(id json) {
       
       if ([json[@"errcode"] integerValue]==0) {
           NSMutableArray *temp = [NSMutableArray array];
           NSMutableArray *temp1 = [NSMutableArray array];
           for (NSDictionary *dic in json[@"rsp"]) {
               SupModel *model = [SupModel new];
               [model setValuesForKeysWithDictionary:dic];
               [temp addObject:model.support_name];
               [temp1 addObject:model.support_id];
           }
           dataArray2 = temp;
           dataArray3 = temp1;
       }
